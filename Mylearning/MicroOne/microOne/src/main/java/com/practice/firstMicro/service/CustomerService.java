...
public class CustomerService {
	...
	public CustomerDto getCustById(Long id) {
		Optional<CustomerEntity> customerEntityOptional =repo.findByCustomerId(id);
		System.out.println("customerEntity"+customerEntityOptional);
		CustomerDto customerDto	=new CustomerDto();
	...
	public List<CustomerDto> getOrdersByCustomerId(Long id) {
		...
		("http://localhost:8081/orders/getOrdersByCustId/"+customerEntity.get().getCustomerId(), OrdersDto[].class);
	...
	public List<CustomerDto> getCartItemsforCustId(Long id)
	{
		Optional<CustomerEntity> customerEntity=repo.findByCustomerId(id);
		List<CustomerDto> CustomerDtoList = new ArrayList<>();
		ResponseEntity<ProductsDto[]> responseEntity= restTemplate.getForEntity("http://localhost:8083/Products/getByUser/"+customerEntity.get().getCustomerId(), ProductsDto[].class);
